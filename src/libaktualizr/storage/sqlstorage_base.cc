...
boost::filesystem::path SQLStorageBase::dbPath() const { return config_.sqldb_path.get(config_.path); }

SQLStorageBase::SQLStorageBase(const StorageConfig& config, bool readonly, std::vector<std::string> schema_migrations,
